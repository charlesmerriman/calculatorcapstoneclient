...
- **Free vs paid carats are tracked separately.** All income accrues to `freeCarats` except the three purchased sources — the Daily Carat Pack's 500-carat purchase bonus, 350 of the paid Training Pass's monthly 2,200, and campaign purchases from the Selectors page — which are the only growth `paidCarats` sees; otherwise it only decreases (via pulls). They combine only for the displayed total. This split is required for discounted paid pulls (paid-only) and for the `full_price_paid_pulls` reserve behavior. A shortfall lands on `freeCarats` (goes negative) rather than paid.
...
- **A campaign purchase's PAID CARATS are credited at an absolute instant** — the campaign's resolved start date — never counted per-window. This is what keeps them tiling; a per-window count would inflate totals the moment a user split their plan across more banners. Gated behind `include_purchases_in_projection`, which is **off by default**, so enabling the feature moves nobody's existing numbers.
- **The SELECTOR TICKETS from that same purchase are banked up front instead**, outside the walk, and are available to every banner in the plan regardless of when the campaign falls. Carats are spent *during* a banner, so December's carats can't buy August's pulls; a selector is not spent at a banner at all — it picks a card out of the back catalogue, and a card stays there after its banner ends. Planning a July banner around a selector you'll hold in December is a real plan. Gating on the campaign date refused it, and since every campaign sits in the future relative to a banner running now, that made selectors nearly unusable. What still constrains a ticket is its **cutoff**, which is calendar-independent. Banking once, outside the walk, is also what stops a ticket being spent twice — the pool is decremented as the walk consumes it.
- **Selector tickets are a bucketed pool, not a scalar, and never fund a pull.** A ticket may only take cards released on JP on or before its cutoff, so two tickets with different cutoffs are different resources — the balance is `{ jpCutoff, count }[]`. A later cutoff strictly dominates an earlier one, so buckets sort weakest-first and spending always takes the *weakest qualifying* ticket, preserving reach. `uma_ticket` / `support_ticket` are gacha tickets worth one pull each and are spent by `applyPullStrategy`; selectors are not and must never enter it, or `maxPossiblePulls` silently inflates.
- **Reserved copies change the odds, not the pull maths.** Copies taken with a selector or an SSR crystal shift the MLB distribution right (`shiftDistribution`) because they are certainties, not probabilities. They never affect `maxPossiblePulls` — crystals and selectors aren't carats. Allocation is selectors-then-crystals (selectors are the constrained resource; crystals are universal), and uma banners can only ever use selectors because this data model has no ★3 crystal.
- **A selector only has to reach ONE card on a banner.** `useBannerResources` derives `oldestFeaturedJpDate` — the *earliest* JP release among the featured cards — and tests the selector's cutoff against that. The ticket takes a single card and the user picks which, so requiring it to clear every card was wrong: until 2026-08 this gated on the *newest* card, and one recent unit made the whole banner read as unfundable (an 11-uma banner with 8 cards inside the cutoff still went red because of the 9th). Cards with no known release date are skipped, so an unknown can neither qualify a banner nor block one; an all-unknown banner yields `null`, which `isCardEligible` refuses under any real cutoff. Note the consequence: green means "this selector can take *something* here", not "it can take the card you want".
- **Over-reserving is reported, not clamped** — mirroring the pull-count input. `reservedFunding.unfunded` drives the red state, and only the *funded* copies reach the odds display.
...
5. If it adds a field to `BannerResources`, add it to **`EMPTY_BANNER_RESOURCES`** too — that frozen object is both the pre-filled result slot and the consumer's fallback, and the two silently drift otherwise.
6. Mirror it in `useAverageMonthlyIncome`, **unless it is a one-off event rather than recurring income** (campaign purchases are the standing exception — averaging them would corrupt the monthly strip).
7. Make sure its payout instants are **absolute** (a fixed date, or an anchor-based cycle like `calculateIntervalOccurrences`), never counted off each window's own start, or the "banner count invariance" tests will fail.

## Shared arithmetic

Two helpers exist specifically so a calculation isn't written twice and left to drift:

- **`sumUncapAccrual`** (`utils/incomeCalculationUtils.ts`) — SSR/SR shards and crystals over a window. Called by both `useUncapCrystals` (one user-chosen window) and `useBannerResources` (one window per banner, chained). The per-banner walk converts shards to crystals *at each checkpoint*, not once at the end, because those crystals must be spendable on reserved copies at that checkpoint.
- **`utils/selectorTickets.ts`** — the bucket arithmetic and the eligibility rule, mirroring `calculatorapi/eligibility.py` exactly, including both edge cases: a null cutoff is unrestricted, and an *unknown* release date is refused under a real cutoff (claiming a selector covers a card it cannot is worse than hiding one it could).
